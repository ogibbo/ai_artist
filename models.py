...
class Generator():
    ...
    def __init__(self, latent_dim: int, image_size: int):
        ...
        # Conv layer to get back to 3 channels for RGB image
        self.model.add(Conv2D(3, 5, padding='same', activation='sigmoid'))
        
# Implementation of the discriminator model
class Discriminator():

    def __init__(self, image_height: int, image_width: int, noise: float):

        # Setting value of noise constant to be later used in the GAN
        self.noise = noise
        ...
        # Single output to enable real/fake classification
...
class GAN(Model):
    ...
    def __init__(self, generator, discriminator):
        ...
        self.noise = discriminator.noise
    ...
    def train_step(self, batch):
        ...
        # Getting the actual data (from the real dataset)
        real_data = batch
        # Creating fake data via the generator model
